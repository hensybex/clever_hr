...
func (u *candidateUsecase) GetCandidateInfo(candidateID uint) (*dtos.CandidateInfo, error) {
	...
	wasResumeAnalysed, err := u.resumeAnalysisResultRepo.WasResumeAnalysed(resume[0].ID)
	if err != nil {
		log.Printf("ERROR: Error fetching resume analysis result check for resume ID: %d. Error: %v", resume[0].ID, err)
