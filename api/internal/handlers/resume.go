...
func (h *ResumeHandler) UploadResume(c *gin.Context) {
	...
	if userIDStr == "" {
		log.Println("Error: User ID is missing from the request")
		c.JSON(http.StatusBadRequest, gin.H{"error": "Telegram ID is required"})
		return
	}
	log.Printf("Processing upload for Telegram ID: %s", userIDStr)
	...
	if err != nil {
		log.Printf("Error: User with UserID %s not found", userIDStr)
